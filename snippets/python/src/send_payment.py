...
def prepare_send_payment_lightning_bolt11(sdk: BindingLiquidSdk):
    ...
    try:
        prepare_response = sdk.prepare_send_payment(PrepareSendRequest(destination=destination))
...
def send_payment(sdk: BindingLiquidSdk, prepare_response: PrepareSendResponse):
    # ANCHOR: send-payment
    try:
        send_response = sdk.send_payment(SendPaymentRequest(prepare_response=prepare_response))

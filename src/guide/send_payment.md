...
<div class="warning">
<h4>Developer note</h4>
When paying an invoice generated by another Liquid SDK instance, or any other application which uses the Boltz swapper internally, the payment will fall back to a direct onchain payment. The advantage of doing so is that the payee will spend less on fees, as they are not relying on external services to execute the payment any longer. To learn more about this process and how it works in detail, see Boltz's documentation for 
<a target="_blank" href="https://docs.boltz.exchange/v/api/magic-routing-hints">Magic Routing Hints (MRH)</a>.
</div>
